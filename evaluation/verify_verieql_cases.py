...
def generate_false_positives(input_path: str, ex_input_path: str):

    df = pd.read_csv(input_path)
    df_ex = pd.read_csv(ex_input_path)
    # merge the two dataframes on question_id
    df = df.merge(df_ex[['question_id', 'res']], on='question_id', how='inner')

    print(df.head())
    mask  = (df["res"] == "correct") & ((df["equivalent"] == "False") | (df["equivalent"] == False))
    df_fp = df[mask]
...
def results_equal(res1: Dict[str, Any], res2: Dict[str, Any]) -> bool:
    
    return set(res1["rows"]) == set(res2["rows"])
...
def run_sqlite_case(setup_sql: str, q1: str, q2: str) -> Dict[str, Any]:
    ...
    if out["q1_ok"] and out["q2_ok"]:
        out["equal"] = "True" if results_equal(res1, res2) else "False"
    else:
        out["equal"] = "Error"
    ...
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        ...
        for r in results:
            ...
            row = {
                ...
                "counterexample": r.get("setup_sql")
            }

            for side in (("generated_sql", "q1"), ("gold_sql", "q2")):
                full = r.get(f"{side[1]}_full")
...
def main():
    ...
    ap.add_argument("input", help="Path to CSV with a 'counterexample' column.")
    ...
    ap.add_argument("--no_boundsize", action = "store_true")
    ap.add_argument("--no_sql_in_thingy", action = "store_true")
    args = ap.parse_args()

    false_positives = generate_false_positives(args.input, args.ex_input)
    ...
    for c in cases:
        ...
        if args.no_sql_in_thingy:
            idx = int(c["question_id"])
            df = pd.read_csv(false_positives)
            mask = (df["question_id"] == idx)
            c["sql1"] = df.loc[mask, "generated_sql"].iloc[0]
            c["sql2"] = df.loc[mask, "gold_sql"].iloc[0]
        ...
        res["gold_sql"] = c["sql2"]
        if res["equal"] in ["False", False] and res["question_id"] not in covered_id:
            results.append(res)
            covered_id.add(res["question_id"])
    ...
    incorrect_attacks = df[df["equal"].isin([True, "True", "Error"])]
    ...
    df_ex = pd.read_csv(args.ex_input)
    # merge the two dataframes on question_id
    df_final = df.merge(df_ex[['question_id', 'res']], on='question_id', how='inner')  
  
    for i, attack in incorrect_attacks.iterrows():
        if args.no_boundsize:
            mask = (df_final["question_id"] == attack["question_id"])
        else:
            mask = (
