...
K = 7  # Maximum bound size to consider
...
def get_bounds_info(question_id, folder, prediction_path, results_rows, max_bound_size):
    bounds = []
    for b in range(max_bound_size):
...
def main():
    ...
    ap.add_argument("--max-bound-size", default=5, type=int, help="Maximum bound size to consider.")
    ...
    with open(input_csv, newline='') as infile, open(output_csv, 'w', newline='') as outfile:
        ...
        for row in reader:
            ...
            if res == "incorrect":
                ...
                bounds = get_bounds_info(question_id, folder, args.prediction, results_rows, args.max_bound_size)
